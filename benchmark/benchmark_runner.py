...
def _measure(
    database: str, phase: str, query_id: str, run: int, operation: Callable[[], int]
) -> dict:
...
def relational_mutations(database: str, phase: str, s: Settings, c) -> list[dict]:
    ...
    if database == "sqlserver":
        statements = {
            "q14_insert": (
                "INSERT dbo.Products(ProductId,CategoryId,Sku,Name,Price,StockQuantity,IsActive,CreatedAt) VALUES(?,1,?,'Benchmark Product',10,10,1,SYSUTCDATETIME())",
                (marker, "BENCH-1"),
            ),
            "q15_update": (
                "UPDATE dbo.Products SET StockQuantity=StockQuantity+1 WHERE ProductId=?",
                (1,),
            ),
            "q16_delete": ("DELETE dbo.Products WHERE ProductId=?", (marker,)),
        }
        ...
        statements = {
            "q14_insert": (
                "INSERT INTO products(product_id,category_id,sku,name,price,stock_quantity,is_active,created_at) VALUES(%s,1,%s,'Benchmark Product',10,10,TRUE,NOW())",
                (marker, "BENCH-1"),
            ),
            "q15_update": (
            ...
            "q16_delete": ("DELETE FROM products WHERE product_id=%s", (marker,)),
        }
    ...
    for query_id, (sql, params) in statements.items():
        for run in range(1, s.repetitions + 1):
            if query_id == "q16_delete":
                c.cursor().execute(seed_sql, (marker, "BENCH-1"))
    ...
    with mongo_connect(s) as client:
        ...
        for query_id, (kind, value) in mongo_pipelines().items():
            ...
            def operation(kind=kind, value=value) -> int:
                if kind == "find_product":
                    return 1 if db.products.find_one(value) else 0
                if kind == "find_name":
                    return len(
                        list(
                            db.products.find(value)
                            .sort([("name", 1), ("_id", 1)])
                            .limit(50)
                        )
                    )
                if kind == "find_category":
                    return len(list(db.products.find(value).sort("_id", 1)))
                if kind == "find_customer_orders":
                    return len(
                        list(
                            db.orders.find(
                                value, {"items": 0, "payment": 0, "shipment": 0}
                            ).sort("orderDate", -1)
                        )
                    )
            ...
            operation()
            for run in range(1, s.repetitions + 1):
                rows.append(_measure("mongodb", phase, query_id, run, operation))
        ...
        for query_id in ("q14_insert", "q15_update", "q16_delete"):
            for run in range(1, s.repetitions + 1):
                db.products.delete_one({"_id": marker})
                if query_id != "q14_insert":
                    db.products.insert_one(base.copy())
                if query_id == "q14_insert":

                    def operation() -> int:
                        return int(db.products.insert_one(base.copy()).acknowledged)

                elif query_id == "q15_update":

                    def operation() -> int:
                        return db.products.update_one(
                            {"_id": marker}, {"$inc": {"stockQuantity": 1}}
                        ).modified_count

                else:

                    def operation() -> int:
                        return db.products.delete_one({"_id": marker}).deleted_count
...
def storage_and_version(database: str, s: Settings) -> dict:
    if database == "sqlserver":
        with closing(sqlserver_connect(s)) as c:
            version = (
                c.cursor()
                .execute("SELECT CAST(SERVERPROPERTY('ProductVersion') AS VARCHAR(30))")
                .fetchone()[0]
            )
            data = (
                c.cursor()
                .execute(
                    "SELECT COALESCE(SUM(CASE WHEN index_id IN (0,1) THEN used_page_count ELSE 0 END)*8*1024,0), 0 FROM sys.dm_db_partition_stats"
                )
                .fetchone()
            )
            index_bytes = (
                c.cursor()
                .execute(
                    "SELECT COALESCE(SUM(used_page_count)*8*1024,0) FROM sys.dm_db_partition_stats WHERE index_id>1"
                )
                .fetchone()[0]
            )
            total = (
                c.cursor()
                .execute("SELECT SUM(size)*8*1024 FROM sys.database_files")
                .fetchone()[0]
            )
    elif database == "postgresql":
        with postgres_connect(s) as c:
            version = c.execute("SHOW server_version").fetchone()[0]
            total = c.execute("SELECT pg_database_size(current_database())").fetchone()[
                0
            ]
            data = c.execute(
                "SELECT COALESCE(SUM(pg_table_size(oid)),0),0 FROM pg_class WHERE relkind='r'"
            ).fetchone()
            index_bytes = c.execute(
                "SELECT COALESCE(SUM(pg_indexes_size(oid)),0) FROM pg_class WHERE relkind='r'"
            ).fetchone()[0]
    else:
        with mongo_connect(s) as client:
            version = client.server_info()["version"]
            stats = client[s.mongo_database].command("dbStats", scale=1)
            data_bytes, index_bytes, total = (
                stats.get("dataSize", 0),
                stats.get("indexSize", 0),
                stats.get("storageSize", 0) + stats.get("indexSize", 0),
            )
            return {
                "database": database,
                "version": str(version),
...
def summarize(raw: list[dict]) -> list[dict]:
    ...
    for row in raw:
        groups.setdefault(
            (row["database"], row["index_phase"], row["query_id"]), []
        ).append(row["elapsed_ms"])
    return [
        {
            "database": db,
            "index_phase": phase,
        ...
        for (db, phase, qid), values in sorted(groups.items())
    ]
