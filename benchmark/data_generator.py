...
class CanonicalGenerator:
    ...
    def categories(self) -> list[tuple]:
        return [
            (i, f"Category {i:02d}", f"Curated products in category {i:02d}")
            for i in range(1, self.s.category_count + 1)
        ]
        ...
        for i in range(1, self.s.customer_count + 1):
            ...
            rows.append(
                (
                    ...
                    first,
                    last,
                    f"{first}.{last}.{i}@example.test".lower(),
                    f"+33-6-{i % 10_000_000:07d}",
    ...
    def products(self) -> list[tuple]:
        return [
            (
                i,
                1 + (i - 1) % self.s.category_count,
                f"SKU-{i:09d}",
                f"Product {i:09d}",
                f"Realistic catalog item {i}",
                product_price(i),
                (i * 37) % 1000,
                True,
                BASE_DATE + timedelta(minutes=i),
            )
            for i in range(1, self.s.product_count + 1)
        ]
    ...
    def order_batches(self) -> Iterator[OrderBatch]:
        ...
        for order_id in range(1, self.s.orders + 1):
            ...
            order_date = BASE_DATE + timedelta(
                seconds=rng.randrange(0, 4 * 365 * 24 * 3600)
            )
            status = ORDER_STATUSES[
                rng.choices(range(6), weights=(5, 12, 12, 15, 52, 4), k=1)[0]
            ]
            chosen = rng.sample(
                range(1, self.s.product_count + 1),
                k=rng.randint(1, min(5, self.s.product_count)),
            )
            ...
            for product_id in chosen:
                ...
                subtotal += line
                order_items.append(
                    (item_id, order_id, product_id, qty, unit, discount, line)
                )
            shipping = Decimal("0.00") if subtotal >= 100 else Decimal("7.50")
            order_discount = (
                money(subtotal * Decimal("0.03"))
                if rng.random() < 0.1
                else Decimal("0.00")
            )
            ...
            payment_status = (
                "failed"
                if status == "cancelled"
                else ("pending" if status == "pending" else "paid")
            )
            paid_at = (
                None
                if payment_status in ("failed", "pending")
                else order_date + timedelta(minutes=2)
            )
            batch.payments.append(
                (
                    order_id,
                    order_id,
                    PAYMENT_METHODS[order_id % 4],
                    payment_status,
                    total,
                    f"TX-{order_id:012d}",
                    paid_at,
            ...
            batch.shipments.append(
                (
                    order_id,
                    order_id,
                    "ParcelCo" if shipped_at else None,
                    tracking,
                    shipment_status,
                    shipped_at,
                    delivered_at,
            ...
            "addresses": [
                {
                    ...
                    "line2": address[4],
                    "city": address[5],
                    "stateProvince": address[6],
                    "postalCode": address[7],
                    "countryCode": address[8],
                    "isDefault": address[9],
                }
            ],
        }
    ...
    @staticmethod
    def mongo_order(
        order: tuple, items: list[tuple], payment: tuple, shipment: tuple
    ) -> dict:
        address_id = order[2]
        ...
        address_snapshot = {
            ...
            "label": "Home",
            "line1": f"{1 + address_id % 999} Market Street",
            "line2": None,
            "city": city,
            "stateProvince": None,
            "postalCode": postal,
        ...
        return {
            ...
            "currency": order[5],
            "subtotal": order[6],
            "shippingAmount": order[7],
            "discountAmount": order[8],
            "totalAmount": order[9],
            "items": [
                {
                    "orderItemId": i[0],
                    "productId": i[2],
                    "productName": f"Product {i[2]:09d}",
                    "quantity": i[3],
                    "unitPrice": i[4],
                    "discountAmount": i[5],
                    "lineTotal": i[6],
            ...
            "payment": {
                ...
                "status": payment[3],
                "amount": payment[4],
                "transactionRef": payment[5],
                "paidAt": payment[6],
            },
            "shipment": {
                "shipmentId": shipment[0],
                "carrier": shipment[2],
                "trackingNumber": shipment[3],
                "status": shipment[4],
                "shippedAt": shipment[5],
                "deliveredAt": shipment[6],
            },
        }
