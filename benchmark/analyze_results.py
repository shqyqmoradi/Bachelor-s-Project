...
def main() -> None:
    ...
    overall = (
        indexed.groupby("database", as_index=False)
        .average_ms.mean()
        .sort_values("average_ms")
    )
    dbs = [
        db
        for db in ("sqlserver", "postgresql", "mongodb")
        if db in set(overall.database)
    ]
    ...
    add_metric(
        "Insert Time (s)",
        lambda db: float(
            inserts.loc[inserts.database == db, "elapsed_seconds"].iloc[0]
        ),
    )
    add_metric(
        "Average Query Time (ms)",
        lambda db: float(
            indexed_raw.loc[indexed_raw.database == db, "elapsed_ms"].mean()
        ),
    )
    add_metric(
        "Minimum Query Time (ms)",
        lambda db: float(
            indexed_raw.loc[indexed_raw.database == db, "elapsed_ms"].min()
        ),
    )
    add_metric(
        "Maximum Query Time (ms)",
        lambda db: float(
            indexed_raw.loc[indexed_raw.database == db, "elapsed_ms"].max()
        ),
    )
    add_metric(
        "Average CPU (%)",
        lambda db: float(
            indexed_raw.loc[indexed_raw.database == db, "avg_cpu_percent"].mean()
        ),
    )
    add_metric(
        "Peak Host RAM (MiB)",
        lambda db: float(
            indexed_raw.loc[indexed_raw.database == db, "peak_ram_mb"].max()
        ),
    )
    add_metric(
        "Total Storage (MiB)",
        lambda db: (
            float(storage.loc[storage.database == db, "total_bytes"].iloc[0]) / 2**20
        ),
    )
    if not backups.empty:
        add_metric(
            ...
            lambda db: float(
                backups.loc[
                    (backups.database == db) & (backups.operation == "backup"),
                    "elapsed_seconds",
                ].iloc[0]
            ),
        )
        add_metric(
            ...
            lambda db: float(
                backups.loc[
                    (backups.database == db) & (backups.operation == "restore"),
                    "elapsed_seconds",
                ].iloc[0]
            ),
        )
    ...
    query_pivot = indexed.pivot(
        index="query_id", columns="database", values="average_ms"
    ).reset_index()
    lines = [
        ...
        "",
        query_pivot.to_markdown(index=False, floatfmt=".3f"),
        "",
    ...
    out = ROOT / "results" / "measured-results.md"
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(out)
